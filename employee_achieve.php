<?php
   ...
   if(isset($_SESSION['userId'])){
    ...
    $q="SELECT * FROM appraisal.achievement_recods where  userId='$idd'";
   ...
   $goal_id=$_SESSION['userId'];
    ...
    $q="SELECT * FROM appraisal.achievement_recods where userId=$Id and achievement= $achienement";
   
   $result = $mysqli->query($q);
   if ($result->num_rows > 0) {        
     $row = $result->fetch_assoc();
      echo  $kpi=$row['assign_kpi'];
...
<html lang="en">
   <head>
      ...
      <style>
         ...
         ul.table_l{margin:0; display:table-cell; width:200px;}
         ul.table_l li{width:100%;}
         
         .table_heading {margin:0}
         .table_heading li{display:table-cell; width:200px;}
      </style>
   </head>
   <body class="achievement" >
      ...
      <main class="page-content">
         <div class="container-fluid tab-content">
            ...
            <table id="goal_table" class="table bg-white">
               <thead>
                  <tr>
                     <th scope="col" style="text-align:right">KPI</th>
                     <th colspan="4" style="padding:0" >
                        <ul class="table_heading">
                           <li scope="col" style="text-align:center " >Target</li>

                           <li scope="col" style="text-align:center">Achievement</li>
                           <li scope="col" style="text-align:center">Achievement %</li>

                           <li scope="col"style="text-align:center"> Date </li>
         ...
         if($assign_kpi!=$row['assign_kpi']){
          $assign_kpi=$row['assign_kpi'];
          echo $row['assign_kpi'];
          }
         else{
            continue;
      ...
      ?> </td>   
        ...
        <?php $qd="SELECT * FROM achievement_recods where userId=$id  and assign_kpi='$assign_kpi'";
         $resultd = $mysqli->query($qd);
         while ($rom = $resultd->fetch_assoc()) {
            ?>
          <?php echo $rom['achievement'];
            echo "</br>";
          }?></td>
...
<td style="text-align:right">
         ...
         while ($rom = $resultd->fetch_assoc()) {
            ?>
          <?php echo $rom['achievement_percent'];
            echo "</br>";
          }?></td>
